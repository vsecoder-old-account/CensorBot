...
async def send_welcome(message: types.Message):
    ...
    time.sleep(1)
    await bot.send_message(message.chat.id, 'Здравствуйте, я бот, который будет удалять маты из чата, если вы заметили что какой-то мат не удаляется, для того чтобы я начал работать, просто добавьте меня в чат, зайдите в настройки, и выдайте мне права администратора!')

@dp.message_handler(commands=['info', 'me'])
async def info(message: types.Message):
    await bot.send_chat_action(message.chat.id, 'typing')
    time.sleep(1)
    await bot.send_message(message.chat.id, 'Ну раз спросил, то в мои возможности входят несколько функций, я обрабатываю все сообщения пользователей, и если это сообщение содержит мат, то я его удаляю, но на всякий случай я присылаю сообщение помеченное вашим именем, но вместо матов три точки, из этого я веду статистику, и приветствую новых пользователей! Так же обрабатываю файлы присылая на их развёрнутую статистику проверки на вирусы для вашей безопастности!')
